These samplers were used for results in the following paper:
...
This is the relevant paper to cite in reference to this code.
...
Note that the variable naming used in the code is not equivalent in many cases to what is used in the paper. The code is thoroughly commented, and the comments describe the correspondences between variable names and model parameters from the paper.
...
- Note that matlab sometimes gives an error when compiling "GibbsSamplerLDA_v2.c" and other files. The issue that this causes has been addressed in the script that uses this code ("Train_pCT_New_V01.m"), so it shouldn't be a problem.
________________________________________________________
INFERENCE DURING TRAINING/TESTING:

The steps required to train the model, and then make predictions using Dependency-LDA are as follows:

TRAINING:

1)  Run the "ldatagmodel_TrainPWC_V01.m" script in "Train_pWC_LabelWord_Probs".  
		Samples z for training documents. Used to train  \Phi, which gives: p(w|c).
	2.1: Run "Compute_GetAvgPWC_New_V01" to infer \Phi for multiple training chains
	2.2: Run "Compute_GetAvgPWC" to compute an average \Phi over all training chains
			
2)  Run the "Train_pCT_New_V01.m" script in "Train_pCT_TopicLabel_Probs".  
...
3)  Run the "DependencyLDA_Sample_TestDocuments.m" script in "/Test_DependencyLDA_TestDocuments".  
...
4) 	Compute final predictions for test-documents and then compute statistics 
	...
	The functions to run within these subdirectories are:
	4.1: To average predictions on test-documents: 
			"TestDocs_Compute_TotalDP_TotalPriors_AllChains_Samples_New_V05"
	4.2: To Compute all stats for test-document predictions: 
...
Changing parameters:
	...
	- Change the Testing Parameters as appropriate (in the DependencyLDA_Sample_TestDocuments.m and PriorLDA_Sample_TestDocuments.m scripts). These parameters in the cell "TEST PARAMETERS" in the matlab script. Note again that the variable names are different from parameter names in the paper, but the relationship is documented in the code.
...
For Dependency-LDA, the code is not really optimized at all (and can be slow for large datasets). Specifically, while the compiled code is fast (for sampling the z and z' variables) it repeatedly passes variables back and forth between matlab and the compiled code. If you would like to make the code run faster, you can try modifying the sampling parameters that control how often the data is passed between matlab and the compiled code (ITERS_LABELS ITERS_TOPICS controls how many full iterations to run on z and z' before the updated values are passed back to matlab). Alternatively (ideally), all of the code would be compiled, but we do not have an implementation of that.

For Prior-LDA and Flat-LDA, the code is quite fast (because we don't sample the z' variables and therefore data does not repeatedly get passed between matlab and the compiled code). 
