%% AVERAGE PREDICTION TOTALS OVER MULTIPLE TEST-CHAINS FOR THE PRIOR-LDA MODEL
clear;clc;close all;
...
testparams.SUMALPHA_LABELS = 70; % TOTAL ALPHA VALUE TO BE TAKEN FROM THE SUPER-TOPICS (equals \eta parameter in paper)
testparams.ADDALPHA_LABELS = 30  ; % ADDITIONAL ALPHA TO DISTRIBUTE EVENLY ACROSS LABELS (equals the sum of \alpha parameter in paper)
...
NCHAINS_PCT      = 1  ; % HOW MANY SETS OF SUPER-TOPIC CHAINS WE ITERATE OVER
NCHAINS_PER_PCT  = 10  ; % HOW MANY TEST CHAINS WE RUN FOR EACH SET SET OF SUPER-TOPICS
...
trainparams.PCT.BETA        = 10 ;
trainparams.PCT.NTOPICS     = 1 ;
...
return
%% Save the totals to the directory containing the results chains
...
%% Temporarily: Create a set of predictions by just adding the prior and totals


%total_doc_preds = total_DC_Alpha + total_DC_Counts;

%save tmpout_nyt_v4
save('tmpout_yahooarts1','total_DC_Alpha','total_DC_Counts');

% save('tmpout_nyt_v4_1topicchain','total_DC_Alpha','total_DC_Counts');


return
%% FOR EACH TOPIC CHAIN, CONVERT ALL SUPER-TOPIC COUNTS INTO PRIORS

% -------- TURN DOCUMENT X TOPIC COUNTS INTO PRIOR DOC X TAG COUNTS ------- %
for pctchain = 1 : npctchains
    
    fprintf( '\n\n =======================================================================\n')
    fprintf('\t  COMPUTING COUNT PRIORS FOR FOR TOPIC-CHAIN %02d \n' , pctchain) ;
    fprintf( ' =======================================================================\n\n')

    NN = pctresults(pctchain).ntotalsamples_pctchain;
    
    % % FIRST CREATE A NORMALIZED DOCUMENT X TAG PROBABILITY MATRIX
    % %normDPT = pctresults(pctchain).totalDPT + DTOPIC_Alpha * NN ;  % ADD THE TOPIC PRIORS BACK INTO DPT
    % % FIXING SHAPE
    % normDPT = pctresults(pctchain).totalDPT + DTOPIC_Alpha' * NN ;  % ADD THE TOPIC PRIORS BACK INTO DPT
    normDPT = pctresults(pctchain).totalDPT + DT_Alpha' * NN ;  % ADD THE TOPIC PRIORS BACK INTO DPT
    normDPT = (normDPT * NN) ./ repmat( sum(normDPT) , nt , 1) ;   % NORMALIZE THE DPT MATRIX SO THAT dPT SUMS TO NN FOR EACH TOPIC
    
    pct_testc = pctresults(pctchain).pct_testc;
    
    % NOW MULTIPLY P(TOPIC|DOCS) AND P(TAGS|TOPICS) TO COMPUTE PRIOR P(TAGS|DOCUMENTS)
    DC_ALPHA = pct_testc * normDPT ;  % MATRIX MULTIPLICATION CREATES A [TAG X DOC] MATRIX FROM [DOC x TOPIC] AND [TOPIC x TAG] MATRICES
    DC_ALPHA = (DC_ALPHA * NN) ./ repmat( sum(DC_ALPHA) , nc , 1) ; % NORMALIZE THE [TAG X DOC PROBABILITY MATRIX]
    
    DC_ALPHA = DC_ALPHA * testparams.SUMALPHA_LABELS ; % MULTIPLY BY SUMALPHA_TAGS TO TURN INTO COUNTS MATCHING THE COUNTS IN ALL CHAINS
    
    pctresults(pctchain).sumalpha_tags = DC_ALPHA;                % THIS IS THE MATRIX WE ADD TO THE DPC COUNTS TO ADD PROPORTIONAL PRIOR COUNTS
    pctresults(pctchain).addalpha_tags = testparams.ADDALPHA_LABELS / nc * NN;   % THIS IS THE FLAT PRIOR WE ADD FOR SMOOTHING (MAY WANT TO LEAVE OUT)
    
    %     %     %     DTAG_Alpha = DTAG_Alpha + ADDALPHA_TAGS / nc * NN; % ADD THE FLAT COUNTS BACK IN
    %     %     %     DTAG_Alpha = DTAG_Alpha' ;      % TRANSPOSE TO MAKE [DOC X TAG] DIMENSIONALITY
    
end

%% Create matrices for storing totalDP and priorPCD

totalDP = zeros(nc,nd);
priorPCD = zeros(nc,nd);

%% Get Full Results

for pctchain = 1 : npctchains
    totalDP = totalDP + pctresults(pctchain).totalDPC ;
    priorPCD = priorPCD + pctresults(pctchain).sumalpha_tags ;
end

%{
for pctchain = 1 : npctchains
    if pctchain==1
        totalDP = pctresults(pctchain).totalDPC ;
        priorPCD = pctresults(pctchain).sumalpha_tags ;
    else
        totalDP = totalDP + pctresults(pctchain).totalDPC ;
        priorPCD = priorPCD + pctresults(pctchain).sumalpha_tags ;
    end
    addAlpha(pctchain) = pctresults(pctchain).addalpha_tags ;
end
%}
...
return
return
%%  CREATE SAVE FILE AND SAVE FILE WITH TOTALS
files.savefilenm_root = sprintf( '%s/TotalDP_SumA%02d_TAGS_SumA%02d_AddA%02d' , files.savedir , testparams.SUMALPHA_TOPICS , SUMALPHA_TAGS , ADDALPHA_TAGS  )  ;
files.savefilename = sprintf('%s_%02dTopicChains_%03dTotalSamples.mat' , files.savefilenm_root  , npctchains , ntestchainsamples )

if ~exist(files.savefilename,'file')
    save(files.savefilename,'totalDP','priorPCD')
else
    error(sprintf('\n\n ********* File Not Saved:  Results File Already Exists!!     *********\n\n'));
end


return

save totalDP_FirstFourChains totalDP  priorPCD addAlpha




%%
%     DTAG_Alpha = DTAG_Alpha ./ repmat( sum(DTAG_Alpha)* , nc , 1) ; % NORMALIZE THE [TAG X DOC PROBABILITY MATRIX] BY THE COUNT TOTAL
                                                                   % MAKE DIVISOR EQUAL THE SUMxSUMALPHA_TAGS RATHER THAN THE SUM --

% pctresults(pctchain).ntestchainsamples = ntestchainsamples ;



% size(DTOPIC_Alpha)
            
%% CODE FOR CONVERTING DOCUMENT-TOPIC ASSIGNMENTS INTO PRIOR PROBABILITIES AND COUNTS
% FOR NOW JUST ADD THE COUNTS AND PRIOR-COUNTS UP
%                 %% FOR EACH TEST-CHAIN, COMPUTE THE PRIOR ON TAGS USING DOCUMENT-TOPIC ASIGNMENTS (ZZ)
%
%                 % -------- TURN DOCUMENT X TOPIC COUNTS INTO PRIOR DOC X TAG COUNTS ------- %
%                 % FIRST CREATE A NORMALIZED DOCUMENT X TAG PROBABILITY MATRIX
%                 normDPT = chaindata.DPT + DTOPIC_Alpha;  % ADD THE TOPIC PRIORS BACK INTO DPT
%                 normDPT = normDPT ./ repmat( sum(normDPT) , nt , 1) ; % NORMALIZE THE DPT MATRIX SO THAT dPT SUMS TO 1 FOR EACH TOPIC
%
%                 % NOW MULTIPLY P(TOPIC|DOCS) AND P(TAGS|TOPICS) TO COMPUTE PRIOR P(TAGS|DOCUMENTS)
%                 DTAG_Alpha = pct_testc * normDPT ;  % MATRIX MULTIPLICATION CREATES A [TAG X DOC] MATRIX FROM [DOC x TOPIC] AND [TOPIC x TAG] MATRICES
%                 DTAG_Alpha = DTAG_Alpha ./ repmat( sum(DTAG_Alpha) , nc , 1) ; % NORMALIZE THE [TAG X DOC PROBABILITY MATRIX]
%                 DTAG_Alpha = DTAG_Alpha * SUMALPHA_TAGS ; % MULTIPLY BY SUMALPHA_TAGS TO TURN INTO COUNTS
%                 DTAG_Alpha = DTAG_Alpha + ADDALPHA_TAGS / nc; % ADD THE FLAT COUNTS BACK IN
%                 DTAG_Alpha = DTAG_Alpha' ;      % TRANSPOSE TO MAKE [DOC X TAG] DIMENSIONALITY


% DECLARE SOME VARIABLE
% DTAG_Alpha  = repmat( (SUMALPHA_TAGS+ADDALPHA_TAGS) / nc , nd , nc);