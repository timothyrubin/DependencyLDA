% function Tag_TestDocs_Using_NYT_DependencyModel_NewV01
clear;clc;close all;

%% SAMPLE TEST DOCUMENTS USING THE PRIOR-LDA MODEL:
% FOR THIS MODEL--WE USE A SINGLE TOPIC CHAIN, WHERE #TOPICS = 1 (THIS
% TOPIC'S DISTRIBUTION OVER LABELS IS PROPORTIONAL TO THE LABELS' TRAININ
% FREQ)
%
%   ** NOTE: YOU NEED TO TRAIN A SINGLE CHAIN USING THE Train_pCT_New_V01.M
%   FILE FOR THIS CODE TO FUNCTION PROPERLY **
%
% TRAINING PARAMETERS:
...
%
% =========== MODEL PARAMETERS USED IN TESTING ===============
...
%   
%   1 - COMPUTE A FIXED DT_Alpha MATRIX OF DOCUMENT->LABEL PRIORS BASED ON THE ONE TOPIC FROM THE TOPIC-CHAIN 
%       (ALL DOCUMENTS WILL HAVE THE SAME PRIOR OVER LABELS PROPORTIONAL TO
%       THE LABELS' TRAINING FREQ)
%   2 - SAMPLE THE Z ASSIGNMENTS OF WORDS->LABELS, CONDITIONED ON THE PRIOR
%   OVER LABELS
...
for pctchain = 1 : NCHAINS_PCT
    ...
    %% Compute the fixed matrix of Document->Label priors \alpha for all docs
    
    % Since there is only one topic, we know that each document will assign all probability to that topic
    DPT = ones(1,nd);
    
    % FIRST CREATE A NORMALIZED DOCUMENT X LABEL PROBABILITY MATRIX
    normDPT = DPT + DT_Alpha';  % ADD THE TOPIC PRIORS BACK INTO DPT
    normDPT = normDPT ./ repmat( sum(normDPT,1) , nt , 1) ; % NORMALIZE THE DPT MATRIX SO THAT dPT SUMS TO 1 FOR EACH TOPIC
    ...
    DC_Alpha = DC_Alpha ./ repmat( sum(DC_Alpha,1) , nc , 1) ; % NORMALIZE THE [LABEL X DOC PROBABILITY MATRIX]
    ...
    for testchain = 1 : NCHAINS_PER_PCT        
        ...
        if ~fileflag
            ...
            % For Prior-LDA, Don't need to do any additional burn-in, or
            % any sampling of ZZ (we assume all ZZ=1 and doc->topic probs
            % is just a Dx1 vectores of ones
            n_iters_total = 0;
            ...
            for SAMPLE = 1 : SAMPLER_NSAMPLES
                ...
                % Don't need to loop over sampling both ZZ and Z: just
                % sample Z's for test-documents after computing the fixed DC_Alpha prior
                
                if SAMPLE == 1
                    % ============== Sample Word-Assignments (Z) starting at their previous assignments, using updated DC_Alpha Matrix ========== %
                    % If first sample: Don't pass in a current z since we haven't initialized z vector yet
                    ...
                    tic;
                    [DPC Z] = GibbsSampler_TagLDA_DCAlpha_V04(testdata.widx, testdata.wdidx, DC_Alpha, pWC , NITER, SEED, OUTPUT);
                else
                    % ============== Sample Word-Assignments (Z) starting at their previous assignments, using updated DC_Alpha Matrix ========== %
                    % If not the first sample, pass in the saved previous state of vector Z
                    fprintf('Topic-Chain %02d; Test-Chain %02d; Sample %02d; Iter %02d - Running Z Sampler (Word->Label assignments) \n' , pctchain , testchain , SAMPLE , n_iters_total);
                    [DPC Z] = GibbsSampler_TagLDA_DCAlpha_V04(testdata.widx, testdata.wdidx, DC_Alpha, pWC , NITER, SEED, OUTPUT , Z);
                end
                
                etime2 = toc;
                SEED = SEED+1;
                n_iters_total = n_iters_total + NITER;
                SAMPLER_ITER = SAMPLER_BURNIN + (SAMPLE-1) * SAMPLER_LAG ;
                assert(SAMPLER_ITER==n_iters_total);
                
                % WRITE FILES AND SAVE VARIABLES
                fprintf( 'Iter %02d Complete. Elapsed time for sampling: %3.1f secs. Approximately %3.1f secs / iteration\n' , n_iters_total, etime2, (etime2) / n_iters_total );
            ...
            savevars_Z_FinalStates( filenm_testchainfinal , Z);
